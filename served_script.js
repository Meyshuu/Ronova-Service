...
const defaultData = {
  ...
  jokiDirectory: [
    {
      name: 'Kira',
      role: 'Carry Specialist',
      rating: '4.9',
      completed: 128,
      status: 'Aktif',
      bio: 'Ahli carry rank dan progress harian.',
      jobs: [
        { title: 'Carry Rank & Progress', status: 'Berhasil' },
        { title: 'Event Clear & Event Trial', status: 'Proses Pengerjaan' }
      ]
    },
    {
      name: 'Alyra',
      ...
      completed: 94,
      status: 'Aktif',
      bio: 'Spesialis farm material dan recovery run.',
      jobs: [
        { title: 'Boss Farm & Material Run', status: 'Berhasil' },
        { title: 'Daily Run & Routine Loop', status: 'Diterima' }
      ]
    },
    {
      name: 'Milo',
      ...
      completed: 63,
      status: 'Onboarding',
      bio: 'Menangani build karakter dan setup team terbaik.',
      jobs: [
        { title: 'Character Build & Team Setup', status: 'Proses Pengerjaan' }
      ]
    }
  ]
};
...
function renderDetail() {
  ...
  document.getElementById('detailJoki').textContent = service.joki;
...
function renderDashboard() {
  ...
  profileSummary.innerHTML = `
    ...
    <p class="body-copy">@${persistedUser.username} ΓÇó ${persistedUser.email || 'email belum diisi'} ΓÇó ${persistedUser.phone}</p>
      ...
      return `
        <article class="order-item">
          <div class="order-head">
            <strong>#${order.id} ΓÇó ${service?.title || 'Layanan'}</strong>
...
function renderAdmin() {
  ...
  if (!admin || admin.role !== 'admin') {
    document.getElementById('jokiDirectory').innerHTML = '<p class="body-copy">Akses admin hanya untuk administrator. Silakan login sebagai admin.</p>';
    document.getElementById('adminOrdersList').innerHTML = '<p class="body-copy">Tidak ada data pesanan untuk ditampilkan.</p>';
    document.getElementById('orderSelect').innerHTML = '<option value="">Tidak ada pesanan</option>';
    return;
  }

  document.getElementById('jokiDirectory').innerHTML = db.jokiDirectory.map((joki) => `
    <div class="joki-item">
      <div class="service-title-row">
        <div>
          <strong>${joki.name} ΓÇó ${joki.role}</strong>
          <div class="item-subtle">Rating ${joki.rating} ΓÇó ${joki.completed} pekerjaan selesai</div>
        </div>
  ...
  document.getElementById('adminOrdersList').innerHTML = db.orders.map((order) => {
    const service = db.services.find((item) => item.id === order.serviceId);
    return `
      <article class="order-item">
        <div class="order-head">
        ...
        <div class="order-body">
          <span class="item-subtle">Pengguna: ${order.userIdTarget} ΓÇó Joki: ${order.assignedJoki}</span>
          <span class="item-subtle">Penugasan: ${order.acceptedByName ? order.acceptedByName : 'Belum diambil'}</span>
          <span class="item-subtle">Budget: ${formatCurrency(order.budget)} ΓÇó ${order.createdAt}</span>
        </div>
      </article>
    `;
  }).join('');
...
attachEvents();
renderAll();
const defaultView = state.currentUser?.role === 'admin' ? 'admin-view' : state.currentUser?.role === 'user' ? 'dashboard-view' : 'home-view';
